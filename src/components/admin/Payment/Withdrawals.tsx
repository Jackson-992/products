...
import { useWithdrawals } from '@/hooks/payments/useWithdrawals.ts';

const Withdrawals = () => {
    const { payments, loading, refetch } = useWithdrawals();
    const [selectedPayment, setSelectedPayment] = useState(null);
    const [isSidebarOpen, setIsSidebarOpen] = useState(false);
    const [searchTerm, setSearchTerm] = useState('');
    const [statusFilter, setStatusFilter] = useState('all');

    // No need to filter since we only get withdrawal payments
    ...
    const filteredPayments = withdrawalPayments.filter(payment => {
        const matchesSearch =
    ...
    if (loading) {
        return <div className="loading">Loading withdrawals...</div>;
    }
            ...
            <div className="table-header">
                <div className="header-left">
                    ...
                    <span className="payment-count">
                        {filteredPayments.length} of {withdrawalPayments.length} payments
                    </span>
                    ...
                    {filteredPayments.length > 0 ? (
                        ...
                        <tr>
                            <td colSpan="5" className="no-results">
                                {hasActiveFilters ? 'No payments match your filters' : 'No withdrawals found'}
                            </td>
                        </tr>
                    )}
                    </tbody>
