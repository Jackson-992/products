...
import { useRegistrationPayments } from '@/hooks/payments/useRegistration.ts';

const RegistrationPayments = () => {
    const { payments, loading } = useRegistrationPayments();
    const [selectedPayment, setSelectedPayment] = useState(null);
    const [isSidebarOpen, setIsSidebarOpen] = useState(false);
    const [searchTerm, setSearchTerm] = useState('');
    const [statusFilter, setStatusFilter] = useState('all');

    // No need to filter since we only get registration payments
    ...
    if (loading) {
        return <div className="loading">Loading registration payments...</div>;
    }

    return (
        <div className="registration-payments-container">
            <div className="table-header">
                <div className="header-left">
                    <h2>Registration Payments</h2>
                    <span className="payment-count">
                        {filteredPayments.length} of {registrationPayments.length} payments
                    </span>
                    ...
                    {filteredPayments.length > 0 ? (
                        ...
                        <tr>
                            <td colSpan="5" className="no-results">
                                {hasActiveFilters ? 'No payments match your filters' : 'No registration payments found'}
                            </td>
                        </tr>
                    )}
                    </tbody>
